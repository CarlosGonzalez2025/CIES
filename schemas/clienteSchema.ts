...
export const clienteSchema = z.object({
  ...
  valor_hora: z.preprocess(
    ...
    z.number().positive('El valor por hora debe ser positivo').optional()
  ),
  porcentaje_comision: z.preprocess(
    (val) => (val === '' ? undefined : Number(val)),
    z.number().min(0).max(1, 'El porcentaje debe estar entre 0 y 1').optional()
  ),
